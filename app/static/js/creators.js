...
export function createCreatorCard(creator) {
  ...
  headerImg.style.cssText = 'width: 100%; aspect-ratio: 3/4; object-fit: cover; background: #f3f4f6; border-radius: 16px 16px 0 0; display: block;';
  ...
  locationContainer.style.cssText = 'min-height: 40px; margin-bottom: 8px; display: flex; flex-direction: column; gap: 4px;';
  
  // Map Hebrew location names to English (in case some are stored in Hebrew)
  ...
  // Helper function to convert location to English
  const getLocationEnglish = (locationValue) => {
    if (!locationValue) return null;
    // If it's already in English (matches known English values), return as-is
    if (['North', 'Haifa', 'Center', 'Tel Aviv', 'Sharon', 'Jerusalem', 'South'].includes(locationValue)) {
      return locationValue;
    }
    // If it's in Hebrew, convert to English
    if (hebrewToEnglishMap[locationValue]) {
      return hebrewToEnglishMap[locationValue];
    }
    // Otherwise return as-is
    return locationValue;
  };
  
  // Helper function to convert comma-separated locations to English
  const getArrivalLocationsEnglish = (arrivalLocationValue) => {
    if (!arrivalLocationValue) return null;
    const locations = arrivalLocationValue.split(',').map(l => l.trim()).filter(l => l);
    return locations.map(loc => getLocationEnglish(loc) || loc).join(', ');
  };
  
  // מיקום יציאה (Departure Location)
  if (creator.location) {
    const locationDisplay = getLocationEnglish(creator.location);
    ...
    location.innerHTML = `
      ...
      <span style="font-weight: 600;">מיקום יציאה:</span> ${locationDisplay}
    `;
    locationContainer.appendChild(location);
  }
  
  // מיקום הגעה (Arrival Location)
  if (creator.arrival_location) {
    const arrivalLocationsDisplay = getArrivalLocationsEnglish(creator.arrival_location);
    const arrivalLocation = document.createElement('div');
    arrivalLocation.style.cssText = 'display: flex; align-items: center; gap: 4px; color: #6b7280; font-size: 12px;';
    arrivalLocation.innerHTML = `
      <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/>
      </svg>
      <span style="font-weight: 600;">מיקום הגעה:</span> ${arrivalLocationsDisplay}
    `;
    locationContainer.appendChild(arrivalLocation);
  }
  ...
  reviewsBtn.style.cssText = 'display: inline-flex; align-items: center; justify-content: center; gap: 6px; padding: 6px 12px; background: #475569; color: white; text-decoration: none; border-radius: 6px; font-size: 12px; font-weight: 600; border: none; cursor: pointer; flex: 1;';
  ...
  if (creator.phone) {
    ...
    whatsappBtn.style.cssText = 'display: inline-flex; align-items: center; justify-content: center; gap: 6px; padding: 6px 12px; background: #128C7E; color: white; text-decoration: none; border-radius: 6px; font-size: 12px; font-weight: 600; flex: 1;';
