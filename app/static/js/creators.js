...
export function createCreatorCard(creator) {
  ...
  // Helper function to convert location to Hebrew
  const getLocationHebrew = (locationValue) => {
    if (!locationValue) return null;
    // If it's already in Hebrew (matches known Hebrew values), return as-is
    if (['צפון', 'חיפה', 'מרכז', 'תל אביב', 'שרון', 'ירושלים', 'דרום'].includes(locationValue)) {
      return locationValue;
    }
    // If it's in English, convert to Hebrew
    if (englishToHebrewMap[locationValue]) {
      return englishToHebrewMap[locationValue];
    }
    // Otherwise return as-is
    return locationValue;
  };
  
  // Helper function to convert comma-separated locations to Hebrew
  const getArrivalLocationsHebrew = (arrivalLocationValue) => {
    if (!arrivalLocationValue) return null;
    const locations = arrivalLocationValue.split(',').map(l => l.trim()).filter(l => l);
    return locations.map(loc => getLocationHebrew(loc) || loc).join(', ');
  };
  
  // מיקום יציאה (Departure Location)
  if (creator.location) {
    const locationDisplay = getLocationHebrew(creator.location);
  ...
  if (creator.arrival_location) {
    const arrivalLocationsDisplay = getArrivalLocationsHebrew(creator.arrival_location);
  ...
  reviewsBtn.style.cssText = 'display: inline-flex; align-items: center; justify-content: center; gap: 6px; padding: 6px 12px; background: #ec4899; color: white; text-decoration: none; border-radius: 6px; font-size: 12px; font-weight: 600; border: none; cursor: pointer; flex: 1;';
  ...
  if (creator.phone) {
    ...
    whatsappBtn.style.cssText = 'display: inline-flex; align-items: center; justify-content: center; gap: 6px; padding: 6px 12px; background: #475569; color: white; text-decoration: none; border-radius: 6px; font-size: 12px; font-weight: 600; flex: 1;';
